...
export async function render(dataView) {
    // modContainer.textContent += "aaaaa"
    // console.log(modContainer)
            ...
            let xAxis = axes[axes.map(axis => axis.name).indexOf("X")];
            let yAxis = axes[axes.map(axis => axis.name).indexOf("Y")];
            let colorAxis = axes[axes.map(axis => axis.name).indexOf("Color")];
            

            // Print the row values.
            const rows = await dataView.allRows();
            
            let xValue, yValue, colorValue;
            ...
            rows.forEach(row => {
                ...
                xValue = 
                xAxis.isCategorical ? row.categorical("X").formattedValue()
                : (row.continuous("X").value())

                yValue =
                yAxis.isCategorical ? row.categorical("Y").formattedValue()
                : row.continuous("Y").value()

                colorValue =
                colorAxis.isCategorical ? row.categorical("Color").formattedValue()
                : row.continuous("Color").value()
                
                modContainer.textContent += `x : ${xValue}, y : ${yValue}, color : ${colorValue} \r\n`
            });
            
            console.log(modContainer.textContent)
